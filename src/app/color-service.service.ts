...
export class ColorServiceService{
  ...
  getFiltersHelper(hexVal : any, tryCount: number){

    // Recursively, get the best result out of 3.
    let result = null;
    ...
    if (rgb){
      ...
      if (tryCount <= 0){
        ...
        let recursiveResult : any = this.getFiltersHelper(hexVal, tryCount - 1);
        if (result.loss < recursiveResult?.loss){
          return result;
        }
        else{
  ...
  getFilters(hexVal : any){
    // Service call
    return this.getFiltersHelper(hexVal, 1);
  }
